...
class AuthSmsController extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        $ch = curl_init();
        /* 여기서부터 수정해주시기 바랍니다. */
        $title = "[MEDIBOX] 본인인증입니다.";
        $message = '안녕하세요. MEDIBOX 입니다. 인증번호는 [$NOTE1] 입니다.';             //필수입력
...
//        $result['auth_code'] = $auth_code;
    ...
    public function check(Request $request)
    {
        ...
        if(empty($auth)) {
            $result['ment'] = '먼저 인증코드를 요청해주세요.';
            return $result;
        }
        if($auth->auth_code != $auth_code) {
            $result['ment'] = '인증코드가 일치하지 않습니다.';
            return $result;
        }
        
        $userInfo = DB::table('user_info')
        ->whereRaw('replace(user_phone, \'-\', \'\') = replace(?, \'-\', \'\')', [$phoneNo])
        ->where([
            ['delete_yn', '=', 'N']
        ])->first();
