...
class NoticeController extends Controller
{
    public function list(Request $request)
    {
        ...
        $contents = DB::table("user_notice")->where($where)
            ->orderBy('ordered', 'desc')
            ->orderBy('create_dt', 'desc')
            ->offset(($pageSize * ($pageNo-1)))->limit($pageSize)
            ->get();
        $count = DB::table("user_notice")->where($where)
            ->count();
    ...
    public function find(Request $request, $id)
    {
        ...
        $contents = DB::table("user_notice")->where([
            ['seqno', '=', $id],
            ['deleted', '=', 'N']
        ])->first();
    ...
    public function add(Request $request)
    {
        ...
        $id = DB::table('user_notice')->insertGetId(
            [
    ...
    public function remove(Request $request, $id)
    {
        ...
        DB::table('user_notice')->where('seqno', '=', $id)->update(
            [
                'deleted' => 'Y', 
                'update_dt' => date('Y-m-d H:i:s') 
            ]
