...
-->
    ...
    function gotoNext(){
        ...
        location.href = '/reservation-modify/payment/{{$reservationInfo->seqno}}?date='+searchDate+'&time='+searchTime+'&serviceId='+targetServiceId;
    }
    ...
    function loadServices(){
        medibox.methods.store.serviceAll({
            partner_seqno: {{$reservationInfo->partner_seqno}},
            store_seqno: {{$reservationInfo->store_seqno}}
...
//                history.back();
    ...
    function toReservationDate(aaa){
        return aaa.substring(0, 16);
    }
    // 예약번호로 정보 호출
    function loadHistory(){
        var data = { user_seqno:{{ $seqno }}, id: {{$reservationInfo->seqno}} };

		medibox.methods.store.reservation.one(data, {{$reservationInfo->seqno}}, function(request, response){
			console.log('output : ' + response);
			if(!response.result){
				alert(response.ment);
				return false;
            ...
            serviceSeqno = response.data.serviceInfo.seqno;

            var targetDt = toReservationDate(response.data.start_dt);
            searchDate = targetDt.split(' ')[0];
            searchTime = targetDt.split(' ')[1];
            targetServiceId = serviceSeqno;

            $('#date_option').text( toReservationDate(response.data.start_dt) + ' 예약' );
