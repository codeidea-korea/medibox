...
@include('user.header2')
    ...
    <section id="res_detail" class="depth02 minishspa">
        <div class="history_wrap">
            <div class="mid">
                <div class="date">
                    <h3>예약 날짜</h3>
                    <ul>
                        <li id="reservation_date">2022년 2월 15일 (수) 14:30</li>
                    </ul>
                </div>
                <div class="store">
                    <h3>예약 매장</h3>
                    <ul>
                        <li id="reservation_shop">{{$reservation_shop}}</li>
                    </ul>
                </div>
                <div class="menu">
                    <h3>예약 메뉴</h3>
                    <ul>
                        <li id="reservation_service">{{$reservation_service}}</li>
                    </ul>
                </div>
                <div class="price">
                    <h3>예약 예상 금액</h3>
                    <span id="reservation_price">{{$reservation_price}}원</span>
                </div>
            </div>
        </div>
        <div class="res_payment_tab">
                    ...
                    <div class="use_coupon_wrap">
                        ...
                        <div class="select_wrap">
                            ...
                            <ul class="option">
                                <li>쿠폰 사용 안함</li>
                                <!--
                                <li>회원 가입 쿠폰 50,000p</li>
                                <li>수능 기념 이벤트 30,000p</li>
                                <li>2022 새해 기념 100,000p</li>
                                -->
                            </ul>
                        </div>                    
                    </div>                
            ...
            <div class="tab_content">
                <div class="service_select_wrap">  
                    ...
                    <div class="holding_point_wrap point_wrap">
                        <h2>보유 정액권</h2>
                        <span class="remain_point2">2,100,000 </span> P
                        <!-- 보유 포인트가 없을 때 -->
                    ...
                    <div class="use_point_wrap point_wrap">
                        <h2>사용 포인트</h2>
                        <span><em id="use_point" class="use_point">242,000</em>P</span>
                        <!-- 사용 포인트가 없을 떄 -->
                        <!-- <span class="empty"><em id="use_point"></em>P</span> -->
                    </div>
                ...
                <div class="res_info_wrap">
                    ...
                    <form action="#" method="post">
                        <div>
                            <h3>이름</h3>
                            <input type="text" id="username2" placeholder="이름을 입력해주세요.">
                        </div>
                        <div>
                            <h3>연락처</h3>
                            <input type="text" id="userphone2" placeholder="연락처를 입력해주세요." pattern="[0-9]{3}[0-9]{4}[0-9]{4}" required>
                        </div>
                    </form>
                </div>
            ...
            <button type="button" id="payment_btn" onclick="save()" class="btn">결제</button>
             <!-- 버튼 활성화 -->
            <!-- <button type="submit" id="payment_btn" class="btn on">결제</button> -->
        </div>
    </section>
    ...
    function cancel(){
        if(confirm('아직 예약이 완료되지 않았습니다. 모든 내용을 취소하고 페이지를 벗어나시겠습니까?')){
            history.back();
        }
    }
    ...
    function setup(){
        ...
        $('#user_name').val("{{$reservationInfo->user_name}}");
        $('#user_phone').val("{{$reservationInfo->user_phone}}");
    }
    ...
    async function save(){
        ...
        medibox.methods.store.reservation.check({
            ...
            var point_type = 'P';
            var price = {{$reservation_old_price}} - {{$reservationInfo->discount_price}};
            var memo = '사용자 예약 수정으로 인한 사용 포인트 반환 (예약번호: [{{$reservationInfo->seqno}}])';
            var data = { admin_seqno:1, user_seqno:{{ $seqno }}, product_seqno: 0, reIssueCoupon: {{$reservationInfo->coupon_seqno}},
            ...
            medibox.methods.point.collect(data, function(request1, response1){
                ...
                var point_type2 = 'P';
                var memo2 = '사용자 예약 수정 (반환된 서비스 포인트: '+medibox.methods.toNumber(price)+')';
                
                var data2 = { admin_seqno:1, user_seqno:{{ $seqno }}, service_seqno: {{$serviceId}}, coupon_seqno: selectedCoupon, discount: discountPrice,
                ...
                medibox.methods.point.use(data2, function(request2, response2){
                    ...
                    }, {{$reservationInfo->seqno}}, function(request3, response3){
                        console.log('output : ' + response3);
                        if(!response3.result){
                            alert(response3.ment);
                            return false;
                        }
                        $('#popup24').addClass('on');
                    }, function(e){
