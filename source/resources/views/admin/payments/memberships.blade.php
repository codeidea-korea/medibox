@php 
$page_title = '쿠폰/바우처/멤버쉽 사용내역';
@endphp
@include('admin.header')

<section class="container">
	<div class="page-title">쿠폰/바우처/멤버쉽 사용내역</div>
		...
		<table>
			...
			<thead>
				<tr>
					...
					<th>사용형태</th>
					<th>쿠폰/바우처명</th>
					<!-- <th>사용범위</th> --> <!-- 해당 내용은 쿠폰만 해당사항이 있어 적용이 어려움 -->
					<th>사용일</th>
					<th>사용기간</th>
				</tr>
			</thead>
	...
	function getList(){
		...
		medibox.methods.point.membership.history.list(data, function(request, response){
			...
			if(response.count == 0){
				$('._tableBody').html('<tr>'
									+'    <td colspan="9" class="td_empty"><div class="empty_list" data-text="내용이 없습니다."></div></td>'
									+'</tr>');
			...
			for(var inx=0; inx<response.data.length; inx++){
				...
				bodyData = bodyData 
							...
							+'	<td>'+(
								...
								: (
									response.data[inx].voucher_name ? '바우처'
									: '멤버쉽'
								)
							)+'</td>'
							+'	<td>'+getHstType(response.data[inx].hst_type)+'</td>'
							+'	<td>'+(
								safetyNull(response.data[inx].service_name)
								+ safetyNull(response.data[inx].voucher_name)
								+ safetyNull(response.data[inx].coupon_name)
							)+'</td>'
							// 사용 범위
							+'	<td>'+response.data[inx].create_dt+'</td>'
							+'	<td>'+response.data[inx].real_start_dt + ' ~ ' + response.data[inx].real_end_dt+'</td>'
							+'</tr>';
			}
