...
<section class="container">
	<div class="page-title">매장 정보 관리</div>
...
// {{session()->get('admin_type')}}
		...
		medibox.methods.store.list(data, function(request, response){
...
//			$('._tableBody > tr > td').not('._tableBody > tr > td[data-action-type=none]').off().on('click', viewInfo);
	...
	function gotoDetail(seq){
		location.href = '/admin/stores/'+seq;
	}
	function addItem(){
		location.href = '/admin/stores/0';
	}		
	function remove(name, seq){
		if(!confirm('\''+name+'\'\n매장을 삭제 하시겠습니까?\n*기존 데이터는 모두 삭제됩니다.')) {
			return;
		}
		medibox.methods.store.remove({}, seq, function(request, response){
			console.log('output : ' + response);
			if(!response.result){
				alert(response.ment);
				return false;
