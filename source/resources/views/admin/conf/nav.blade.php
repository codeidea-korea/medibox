...
@php 
$navData = [];
$navData['회원관리'] = ['회원관리', '회원등록', '회원수정', '포인트 자동 적립관리'];


$navData['브랜드 관리'] = ['브랜드 정보', '매장 정보 관리', '예약가능시간 관리', '디자이너 정보', '서비스 정보'];
$navData['브랜드 정보'] = ['브랜드 정보', '제휴사 등록', '제휴사 수정'];
$navData['매장 정보 관리'] = ['매장 정보 관리', '매장 등록', '매장 수정'];
...
$navData['결제 사용내역'] = ['충전/결제내역', '쿠폰/바우처/멤버쉽 사용내역'];
$navData['충전/결제내역'] = ['충전/결제내역'];
	...
	<div class="header_container">
		...
		<nav id="nav">
			<ul id="nav_ul">
				...
				@endphp
				<li class="@if (in_array($page_title, $navData['브랜드 관리'])) active @endif">
					<a href="#" class="mont">브랜드 관리</a>
					<ul>
						@php
						if(session()->get('admin_type') != 'S') {
						@endphp
						<li class="@if (in_array($page_title, $navData['브랜드 정보'])) active @endif"><a href="/admin/partners">브랜드 정보</a></li>
						@php
						}
						@endphp
						<li class="@if (in_array($page_title, $navData['매장 정보 관리'])) active @endif"><a href="/admin/stores">매장 정보 관리</a></li>

						<li class="@if (in_array($page_title, $navData['예약가능시간 관리'])) active @endif"><a href="/admin/business-hours">예약가능시간 관리</a></li>
				...
				<li class="@if (in_array($page_title, $navData['회원관리'])) active @endif">
					...
					<ul>
						...
						<li style="display:none;" class="@if ($page_title == '포인트 자동 적립관리') active @endif"><a href="#">포인트 자동 적립관리</a></li>
					</ul>
				</li>
				<li class="@if (in_array($page_title, $navData['결제 사용내역'])) active @endif">
					<a href="#" class="mont">결제 사용내역</a>
					<ul>
						<li class="@if (in_array($page_title, $navData['충전/결제내역'])) active @endif"><a href="/admin/payments/point">충전/결제내역</a></li>
