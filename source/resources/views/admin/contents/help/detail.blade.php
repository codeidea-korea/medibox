...
@php 
$page_title = $id == 0 ? '도움말 등록' : '도움말 수정';
@endphp
@include('admin.header')

<section id="wrtie" class="container">

	<div class="section-header">도움말 @php echo $id == 0 ? '등록' : '상세'; @endphp</div>
	...
	<script>
		var userId;
	function cancel(){
		window.location.href = '/admin/contents/helps';
	}
	...
	function add(){
		...
		medibox.methods.contents.help.add({
			title: title
			, contents: contents
			, admin_seqno: {{ $seqno }}
    ...
    */
	function getInfo(){
		...
		medibox.methods.contents.help.one(data, '{{ $id }}', function(request, response){
			console.log('output : ' + response);
			if(!response.result){
				alert(response.ment);
				return false;
...
$('#title').val( response.data.title );
	...
	function remove(){
		...
		medibox.methods.contents.help.remove({
			id: userId
