...
@php 
$page_title = $id == 0 ? '공지사항 (유저/APP) 등록' : '공지사항 (유저/APP) 수정';
@endphp
@include('admin.header')

<section id="wrtie" class="container">

	<div class="section-header">공지사항 (유저/APP) @php echo $id == 0 ? '등록' : '상세'; @endphp</div>
	...
	function add(){
		...
		medibox.methods.contents.notice.add({
			title: title
			, contents: contents
			, admin_seqno: {{ $seqno }}
    ...
    */
	function getInfo(){
		...
		medibox.methods.contents.notice.one(data, '{{ $id }}', function(request, response){
			console.log('output : ' + response);
			if(!response.result){
				alert(response.ment);
				return false;
...
$('#title').val( response.data.title );
	...
	function remove(){
		...
		medibox.methods.contents.notice.remove({
			id: userId
