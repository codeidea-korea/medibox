...
@php 
$page_title = $id == 0 ? '개인정보약관 등록' : '개인정보약관 수정';
@endphp
...
<section id="wrtie" class="container">
	...
	<script>
		var userId;
	function cancel(){
		window.location.href = '/admin/contents/privacies';
	}
	...
	function add(){
		...
		medibox.methods.contents.privacy.add({
			title: title
			, contents: contents
			, admin_seqno: {{ $seqno }}
	...
	function getInfo(){
		...
		medibox.methods.contents.privacy.one(data, '{{ $id }}', function(request, response){
			console.log('output : ' + response);
			if(!response.result){
				alert(response.ment);
				return false;
...
$('#title').val( response.data.title );
	...
	function remove(){
		...
		medibox.methods.contents.privacy.remove({
			id: userId
